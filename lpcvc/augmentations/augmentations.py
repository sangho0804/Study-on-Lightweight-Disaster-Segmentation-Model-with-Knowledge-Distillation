...
class RandomScale(object):
    def __init__(self, scale=[0.75,2]):
        self.sscale = scale[0]
        self.lscale = scale[1]
    ...
    def __call__(self, img, mask):
        ...
        w = int(random.uniform(self.sscale, self.lscale) * img.size[0])
        h = int(random.uniform(self.sscale, self.lscale) * img.size[1])
...
class Scale(object):
    ...
    #print('--------SCALE----------')
        if label is not None:
            ...
            label = label.resize((self.size[1],self.size[0]), Image.NEAREST)
        image = image.resize((self.size[1],self.size[0]), Image.BILINEAR)
        return image, label
       


class Color_Normalize(object):
	...
	#print('--------Color_Normalize----------')
        image = np.array(image)
        image = (image.astype(float)-self.mean)/self.std
        return image, label

class Normalize(object):
    def __init__(self, base=255.0):
        self.base = base
    def __call__(self, image, label=None):
	#print('--------Normalize----------')
        image = np.array(image)
        image = image.astype(float) / self.base
        return image, label
